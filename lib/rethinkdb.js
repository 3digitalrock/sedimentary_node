...
module.exports.getFeatured = function (playlist, callback) {
  onConnect(function (err, connection) {
    r.db(dbConfig['db']).table('featured').filter(r.row('playlists').contains(playlist)).eqJoin('uid', r.db('dev').table('videos')).zip().pluck('uid','files','thumbnails').run(connection, function(err, cursor) {
      if(err) {
        callback(err, null);
      } else {
