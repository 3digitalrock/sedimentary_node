...
module.exports.getFeatured = function (playlist, callback) {
  onConnect(function (err, connection) {
    r.db(dbConfig['db']).table('featured').filter(r.row('playlists').contains(playlist)).eqJoin('uid', r.db(process.env.NODE_ENV+'_api').table('trailers')).zip().pluck('uid','title','files','thumbnails','tech').run(connection, function(err, cursor) {
      if(err) {
        callback(err, null);
      } else {
        cursor.toArray(function(err, results) {
