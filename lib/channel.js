...
module.exports.videoPage = function(req, res, html) {
    apiClient.getVideo(req.params.video, function(status, results){
        if(status!==200){
            ...
            if(results.youtube){
                if(!html){
                    res.render('video-yt', {atVideo:true, video: results, layout: 'channels', pageTitle: results.title, youtube: true });
                } else {
                    res.render('video-yt', {atVideo:true, video: results, ajax: true, layout: false});
                }
            } else {
                if(!html){
                    res.render('video-h5', {atVideo:true, video: results, layout: 'channels', pageTitle: results.title });
                } else {
                    res.render('video-h5', {atVideo:true, video: results, ajax: true, layout: false});
                }
            }
        }
    });
