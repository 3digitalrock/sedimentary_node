...
var s3UploadService = function(req, next) {
  ...
  req.busboy.on('file', function(fieldname, file, filename, encoding, mimetype) {
    ...
    file.on('end', function() {
      ...
      // Create filename
      var hashFilename = crypto.randomBytes(Math.ceil(12/2)).toString('hex').slice(0,12);

      var pathToVideo = '/videos/original/' + hashFilename + filename.substr(filename.length - 4);
