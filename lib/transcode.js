...
module.exports.createJob = function(fileAndPath, fileName){
    client.Job.create({
          ...
          "input": "s3://"+config.aws.S3_BUCKET_NAME+fileAndPath,
          "outputs": [
            {
              "label": "mp4_high",
              "url": "s3://"+config.aws.S3_BUCKET_NAME+"/videos/transcoded/"+fileName+"-high.mp4",
              "h264_profile": "high",
              "public": true
            },
            {
              "url": "s3://"+config.aws.S3_BUCKET_NAME+"/videos/transcoded/"+fileName+".webm",
              "label": "webm",
              "format": "webm",
              "public": true
            },
            {
              "url": "s3://"+config.aws.S3_BUCKET_NAME+"/videos/transcoded/"+fileName+".ogg",
              "label": "ogg",
              "format": "ogg",
              "public": true
            },
            {
              "url": "s3://"+config.aws.S3_BUCKET_NAME+"/videos/transcoded/"+fileName+"-low.mp4",
              "label": "mp4_low",
              "size": "640x480",
              "public": true
            },
              ...
              "thumbnails": [
                {
                  ...
                  "number": 3,
                  "base_url": "s3://"+config.aws.S3_BUCKET_NAME+"/videos/thumbnails/",
...
module.exports.jobCallback = function(result){
    if(result.job.state==="finished"){
      api_client.getVideo({id: result.job.pass_through}, function(status, item){
        ...
        api_client.updateVideo(result.job.pass_through, updates);
      });
    }
};