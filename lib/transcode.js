...
module.exports.createJob = function(fileAndPath, fileName, videoType){
    ...
    client.Job.create({
          "notifications": [
              {"format": "json", "url": "https://"+process.env.DOMAIN+"/transcode_callback"},
          ],
...
module.exports.jobCallback = function(result){
    ...
    if(result.job.state==="finished"){
      api_client.getVideo({id: result.job.pass_through.filename}, function(status, item){
        ...
        var urlRegex = /http:\/\/slate\.3digitalrockstudios\.com\.s3\.amazonaws\.com/g;
        var newUrl = "https://slate.3digitalrock.com";
        item["files"] = {};
        item["thumbnails"] = [];
            ...
            if(obj.thumbnails){
              obj.thumbnails.forEach(function(size){
                for(var count = 0; count < size.images.length; count++){
                  var itemUrl = size.images[count].url.replace(urlRegex, newUrl);
                  item["thumbnails"].push(itemUrl);
                }
                item["thumbnails"]["default"] = item["thumbnails"][0];
              });
            } else {
                // For everything else, there's MasterCard.
                var itemUrl = obj.url.replace(urlRegex, newUrl);
                item["files"][obj.label] = itemUrl;
            }
        });
        
        aws.s3DeleteService([result.job.pass_through.fullpath]);
        if(result.job.pass_through.videotype==="videos")item["status"] = '2';
