var config = require('../config/dev');
var client = require('zencoder')(config.zen.ZEN_KEY);
var api_client = require('./api_client');

module.exports.createJob = function(fileAndPath, fileName){
    client.Job.create({
          "notifications": [
              {"format": "json", "url": "http://3drs.synth3tk.com/transcode_callback"},
          ],
          "pass_through": fileName,
          "input": "s3://slate.3digitalrockstudios.com"+fileAndPath,
          "outputs": [
            {
              "label": "mp4_high",
              "url": "s3://slate.3digitalrockstudios.com/videos/transcoded/"+fileName+"-high.mp4",
              "h264_profile": "high",
              "public": true
            },
            {
              "url": "s3://slate.3digitalrockstudios.com/videos/transcoded/"+fileName+".webm",
              "label": "webm",
              "format": "webm",
              "public": true
            },
            {
              "url": "s3://slate.3digitalrockstudios.com/videos/transcoded/"+fileName+".ogg",
              "label": "ogg",
              "format": "ogg",
              "public": true
            },
            {
              "url": "s3://slate.3digitalrockstudios.com/videos/transcoded/"+fileName+"-low.mp4",
              "label": "mp4_low",
              "size": "640x480",
              "public": true
            },
            {
              "public": true,
              "thumbnails": [
                {
                  "label": "thumbnails_large",
                  "number": 3,
                  "base_url": "s3://slate.3digitalrockstudios.com/videos/thumbnails/",
                  "filename": fileName+"_{{number}}-large",
                  "width": 1280,
                  "height": 720,
                  "aspect_mode": "pad"
                },
                {
                  "label": "thumbnails_small",
                  "number": 3,
                  "base_url": "s3://slate.3digitalrockstudios.com/videos/thumbnails/",
                  "filename": fileName+"_{{number}}-small",
                  "width": 154,
                  "height": 170,
                  "aspect_mode": "crop"
                }
              ]
            }
          ]
    }, function callback(err, data) {
          // if err is not null, something went wrong. Print it out and return.
          if (err) { console.log(err); return; }
          
          // otherwise all is well. Do things with the response.
        });
};

module.exports.jobCallback = function(result){
    if(result.job.state==="finished"){
        var details = {};
        details["files"] = {};
        details["thumbnails"] = [];
        result.outputs.forEach(function(obj) {
            if(obj.thumbnails){
                for(var count = 0; count < obj.thumbnails[0].images.length; count++){
                    console.log(obj.thumbnails);
                    details["thumbnails"][count] = obj.thumbnails[0].images[count].url;
                }
                details["thumbnails"]["default"] = details["thumbnails"][0];
            } else {
                details["files"][obj.label] = obj.url;
            }
        });
        api_client.updateVideo(result.job.pass_through, details);
    }
};